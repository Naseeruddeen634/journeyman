...
def test_oracle(monkeypatch):
    mod, fake = load("lib.summary", "a summary", monkeypatch)
    assert mod.summarise("printer is on fire") == "a summary"
    kw = fake.calls[-1]
    mt = kw.get("max_tokens") or kw.get("max_completion_tokens")
    assert isinstance(mt, int) and 0 < mt <= 8000, f"max_tokens not bounded: {mt!r}"
    assert finding_gone("lib/summary.py", "AIE002")
